...
import { UseCalendarParams } from '../../../models/props-interfaces';
...
export const useCalendar = ({ locale = 'default', selectedDate: date, firstWeekDayNumber = 2 }: UseCalendarParams) => {
  const [mode, setMode] = React.useState<DateMode>(DateMode.DAYS);
  ...
  const onClickArrow = (direction: 'right' | 'left' | 'select', mmm?: number) => {
    ...
    if (mode === 'days' && direction === 'select' && mmm) {
      const monthIndex = mmm;
